...
class Store:
    ...
    def upsert_triggers(self, cur, rows: list[dict]):
        ...
        cur.executemany(
            self.q(
                """
                insert into triggers (id, topic_id, news_id, kind, event,
                                      strength, detected_at, expires_at, note)
                values (?, ?, ?, ?, ?, ?, ?, ?, ?)
                on conflict (id) do update set
                  kind = excluded.kind,
            ...
            [(trigger_uuid(r["topic_slug"], r["url"]), topic_uuid(r["topic_slug"]),
              news_uuid(r["url"]), r.get("kind"), r.get("event"),
              r.get("strength"),
    ...
    def active_triggers(self, now: dt.datetime) -> dict:
        ...
        with self.cursor() as cur:
            cur.execute(
                self.q(
                    "select t.slug, g.kind, g.event, g.strength, g.detected_at, "
            ...
            rows = cur.fetchall()
        out: dict = {}
        for (slug, kind, event, strength, detected, expires, note,
             title, url, source) in rows:
            if slug in out:
